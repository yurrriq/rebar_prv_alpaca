%% NOTE: This module was originally called rebar_test_utils in rebar3
%% and has been slightly modified for rebar_prv_lfe.

-module(rebar_prv_lfe_test_utils).
-include_lib("common_test/include/ct.hrl").
-include_lib("eunit/include/eunit.hrl").
-export([init_rebar_state/1, init_rebar_state/2,
         run_and_check/4, check_results/3]).
-export([expand_deps/2, flat_deps/1, top_level_deps/1]).
-export([create_lfe_app/4, create_lfe_erl_app/4, create_plugin/4, 
         create_eunit_app/4, create_empty_app/4,
         create_config/2, create_config/3, package_app/3]).
-export([create_random_name/1, create_random_vsn/0,
         write_lfe_src_file/2, write_erl_src_file/2]).
...
init_rebar_state(Config, Name) ->
    ...
    application:load(rebar_prv_lfe),
    ...
    State0 = rebar_state:new([{base_dir, filename:join([AppsDir, "_build"])}
                            ,{global_rebar_dir, GlobalDir}
                             ,{root_dir, AppsDir}]),
    {ok, State} = rebar_prv_lfe:init(State0),
...
create_lfe_app(AppDir, Name, Vsn, Deps) ->
    write_lfe_src_file(AppDir, Name ++ ".lfe"),
    write_lfe_src_file(AppDir, "not_a_real_src_" ++ Name ++ ".lfe"),
...
%% - src/<file>.app.src
%% And returns a `rebar_app_info' object.
create_lfe_erl_app(AppDir, Name, Vsn, Deps) ->
    write_erl_src_file(AppDir, Name ++ ".erl"),
    write_erl_src_file(AppDir, "not_a_real_src_" ++ Name ++ ".erl"),
    write_app_src_file(AppDir, Name, Vsn, Deps),
    rebar_app_info:new(Name, Vsn, AppDir, Deps).
...
write_lfe_src_file(Dir, Name) ->
    Erl = filename:join([Dir, "src", Name]),
    ok = filelib:ensure_dir(Erl),
    ok = ec_file:write(Erl, lfe_src_file(Name)).

write_erl_src_file(Dir, Name) ->
    Erl = filename:join([Dir, "src", Name]),
    ok = filelib:ensure_dir(Erl),
...
lfe_src_file(Name) ->
    io_lib:format("(defmodule ~s\n"
                  "  (export (main 0)))\n"
                  "(defun main () 'ok)\n",
                  [filename:basename(Name, ".lfe")]).
